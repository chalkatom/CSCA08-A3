...
Toronto is the best university.'''},
    ...
    def test_noauthors(self):
        """Test get_most_published_authors if there are no authors."""

        arxiv_copy = deepcopy(self.exampleb)
        expected = []
        actual = get_mpas(self.exampleb)
        msg = message(arxiv_copy, expected, actual)
        self.assertEqual(actual, expected, msg)
    # TODO: add a complete test suite here

    def test_noties(self):
        """Test get_most_published_authors if there is only one author
        with the most publications."""

        arxiv_copy = deepcopy(self.examplec)
        expected = [('Tafliovich', 'Anya Y.')]
        actual = get_mpas(self.examplec)
        msg = message(arxiv_copy, expected, actual)
        self.assertEqual(actual, expected, msg)
